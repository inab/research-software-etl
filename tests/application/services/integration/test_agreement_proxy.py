...
class FakeHuggingFaceClient:
    """Stands in for HuggingFaceClient; returns a canned llama response."""

    def __init__(self, response):
        self.response = response

    def query_chat(self, messages, model, provider):
        return self.response, {}


class FakeOpenRouterClient:
    """Stands in for OpenRouterClient; returns a canned mixtral response."""
...
def fake_clients(llama_response, mixtral_response):
    """The proxy only touches .huggingface and .openrouter, so the GitHub and
    GitLab slots stay None -- if the proxy ever reaches for them, that's a bug
    and the test should blow up loudly."""
    return build_fake_clients(
        huggingface=FakeHuggingFaceClient(llama_response),
        openrouter=FakeOpenRouterClient(mixtral_response),
    )


@pytest.mark.parametrize("mock_llama, mock_mixtral, expected_verdict", test_cases)
def test_decision_agreement_proxy_with_variants(mock_llama, mock_mixtral, expected_verdict):
    clients = fake_clients(mock_llama, mock_mixtral)
