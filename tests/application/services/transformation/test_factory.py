import pytest
from application.services.transformation.standardizers_factory import (
...
def test_factory_returns_correct_standardizer(source, expected_class):
    ...
    assert isinstance(
        standardizer, expected_class
    ), f"Factory should create an instance of {expected_class.__name__} for source '{source}'"
...
def test_factory_raises_error_for_invalid_source():
    ...
    assert "No standardizer available for source UnknownSource" in str(
        excinfo.value
    ), "Expected ValueError for unknown sources"
