...
def score_one_tool(repos: Repositories, entry: dict, force: bool = False) -> str:
    """
    Score a single tool and upsert its FAIR result.

    Returns one of ``"processed"``, ``"skipped"`` or ``"failed"`` so both the
    batch loop and the per-record ``enrich-tool`` path can share the exact same
...
def add_fair_scores(
    ...
    try:
        for entry in tools:
            status = score_one_tool(repos, entry, force=force)
            if status == "processed":
                processed += 1
            elif status == "failed":
                failed += 1
