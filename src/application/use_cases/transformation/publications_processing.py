...
from application.services.publications.publication_standardizer_factory import (
    StandardizerFactory,
)
from application.services.publications.publication_extractor_factory import (
    ExtractorFactory,
)
...
def publication_in_collection(
    publication: Dict[str, Any], publications_repo: PublicationRepository
) -> Optional[str]:
    """
...
def add_publication(
    ...
    # Insert in database
    logger.debug(
        f"Adding publication {metadata_dict['data']['title']} to the publications collection."
    )
...
def standardize_publications(
    ...
    publication_id = publication_in_collection(
        standardized_publication_dict, publications_repo
    )
    if publication_id:
        publications_ids.add(publication_id)
    else:
        publication_id = add_publication(
            standardized_publication_dict, publications_repo, config
        )
        publications_ids.add(publication_id)
