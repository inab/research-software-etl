...
def map_license(name: str) -> str:
    ...
    if (
        "unlicensed" in name_lower
        or "unknown" in name_lower
        or "unlicense" in name_lower
    ):
        return "unlicensed"
...
def count_tools_per_license(
    ...
    for entry in tools:
        ...
        if (
            "opeb_metrics" in entry.get("source", [])
            and "not-software" in mapped_licenses
        ):
    ...
    count_unambiguous["Open Source"] = len(list_coinc) - count_unambiguous.get(
        "other", 0
    )
...
def licenses_summary_sunburst(
    ...
    count_unambiguous: Dict[str, int],
    collection: str,
    ...
    licenses_ids = [
        k for k in licenses_parents if k in license_summary or k in count_unambiguous
    ]
    ...
    data = {
        ...
        "v": [
            license_summary.get(k, count_unambiguous.get(k, 0)) for k in licenses_ids
        ],
