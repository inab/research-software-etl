...
class CitationSource(BaseModel):
    ...
    citations_per_year: List[YearlyCitation] = Field(
        default={}, description="Citations count per year"
    )
...
class Publication(BaseModel):
    ...
    pmcid: Optional[str] = Field(
        pattern=r"^PMC\d+$", default=None, description="PubMed Central ID"
    )
    pmid: Optional[str] = Field(
        pattern=r"^\d+$", default=None, description="PubMed Central ID"
    )
    ...
    citations: Optional[List[CitationSource]] = Field(
        None, description="Citation data from various sources"
    )
