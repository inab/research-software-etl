...
class source_item(BaseModel):
    ...
    source_url: Optional[HttpUrl] = Field(
        None, description="URL of the source"
    )  # generally, only available for github, sourceforge and bioconductor
...
class Metadata(BaseModel):
    ...
    id: str = Field(
        ...,
        description="Identifier of the entry in the database. It is transform to _id right before insertion.",
    )
    created_at: str = Field(
        ..., description="Creation date of the entry", alias="created_at"
    )
    created_by: str = Field(
        ..., description="User or task that created the entry", alias="created_by"
    )
    created_logs: str = Field(
        ...,
        description="Link to the logs of the creation process",
        alias="created_logs",
    )
    last_updated_at: str = Field(
        ..., description="Last update of the entry", alias="last_updated_at"
    )
    updated_by: str = Field(
        ..., description="User or task that updated the entry", alias="updated_by"
    )
    updated_logs: Optional[str] = Field(
        ..., description="Link to the logs of the last update", alias="updated_logs"
    )
    source: List[source_item] = Field(
        ..., description="Sources of the entry", alias="source"
    )  # only one source!
...
class VersionedMetadata(Metadata):
    ...
    version: int
    source: source_items_list = Field(
        ..., description="Source of the entry", alias="@source"
    )
