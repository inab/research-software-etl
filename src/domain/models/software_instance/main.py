...
from domain.models.software_instance.topic_operation import (
    vocabulary_topic,
...
class instance(BaseModel, validate_assignment=True):
    ...
    type: Optional[software_types] = Field(
        None,
    ...
    download: List[AnyUrl] = Field(
        ...
        example=[
            "https://bioconductor.org/packages/3.16/bioc/src/contrib/simpleSeg_1.0.0.tar.gz"
        ],
    )
    ...
    test: bool = Field(
        ...
        example=True,
    )  # WARNING: this is a list of links in Observatory API --> incompatibility
    inst_instr: bool = Field(
        False,
    ...
    contribution_policy: bool = Field(
        False,
    ...
    input: List[data_format] = Field(
        [], title="Input", description="List of input data formats of the software."
    )
    output: List[data_format] = Field(
        [], title="Output", description="List of output data formats of the software."
    )
    ...
    license: List[license_item] = Field(
    ...
    authors: List[contributor] = Field(
        [], title="Authors", description="List of authors of the software."
    )
    topics: List[vocabulary_topic] = Field(
        [], title="Topics", description="List of topics of the software."
    )
    operations: List[vocabulary_operation] = Field(
        [], title="Operations", description="List of operations of the software."
    )
    sources_labels: dict = {}
    publication: List[ObjectId] = Field(
        [], title="Publication", description="List of DB Ids of the software."
    )
    ...
    citation: List[dict] = Field(
        default_factory=list, title="Citation", description="How to cite the software."
    )
    ...
    def clean_links(cls, value) -> List[AnyUrl]:
        ...
        for link in value:
            if link.path:
                x = re.search(
                    "^(.*)(\\.)(rar|bz2|tar|gz|zip|bz|json|txt|js|py|md)$", link.path
                )
                if x:
    ...
    def clean_webpage(cls, value) -> List[str]:
        ...
        if not value:
            ...
            webpage = [url for url in value if not str(url).startswith("ftp://ftp.")]
    ...
    def compose_webpage(cls, value) -> List[str]:
        ...
        if value:
            for link in value:
                if link.path:
                    x = re.search(
                        "^(.*)(\\.)(rar|bz2|tar|gz|zip|bz|json|txt|js|py|md)$",
                        link.path,
                    )
                    if not x:
