...
def _canonicalize(value: Any) -> Any:
    ...
    if isinstance(value, list):
        ...
        return sorted(
            canon, key=lambda item: json.dumps(item, sort_keys=True, default=str)
        )
    return value
...
def content_hash(data: dict) -> str:
    ...
    """
    payload = json.dumps(
        _canonicalize(data), sort_keys=True, default=str, ensure_ascii=False
    )
...
def validate_and_filter(instance_cls, **data):
    ...
    try:
        ...
        logger.warning(
            f"Could not validate the entire entry. Some fields will be excluded: {e}"
        )
        for error in e.errors():
            logger.warning(
                f"Could not validate a filed. It will be excluded from the entry: {error}"
            )
